...
// Função para carregar o projeto (tenta carregar do servidor, se falhar, usa localStorage)
// function carregarProjeto() {
// 	fetch("http://localhost:3000/api/projeto/1")
...
// 		.catch((error) => {
// 			console.error(
// 				"Erro ao carregar do servidor, carregando localmente.",
// 				error
// 			);
// 			const projeto = localStorage.getItem("projetoPlanta");
// 			if (projeto) {
// 				canvas.loadFromJSON(projeto, function () {
// 					canvas.renderAll();
// 					notificar("Projeto carregado localmente.");
// 				});
// 			} else {
// 				notificar("Nenhum projeto salvo encontrado.", "red");
// 			}
// 		});
// }
...
async function atualizarProjeto() {
	try {
		const response = await fetch(
			`http://localhost:3000/api/projeto`,
			{
				method: "PUT",
				headers: {
					"Content-Type": "application/json",
